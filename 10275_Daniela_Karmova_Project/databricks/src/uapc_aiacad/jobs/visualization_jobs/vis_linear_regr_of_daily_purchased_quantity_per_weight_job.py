...
columns_to_convert_to_pandas_dtypes = ["max_daily_quantity_per_weight", "min_daily_quantity_per_weight"]


class VisDailyDistributedQuantityPerWeightPredictJob(AbstractPredictJob):
    ...
    def _transform(self, read_dfs):
        ...
        logger.info("Creating new df with grouped date of my period"
                    "and find max and min daily distributed quantity per concrete unit of measure - in this case 'KG'")

        filtered_df_sales_quntity_per_weight = find_sum_quantity_per_certain_date(df=filtered_df_sales,
                                                                                  group_col=date_col,
                                                                                  sum_col=quantity_col,
                                                                                  unit_of_measure=unit_of_measure)

        logger.info("Creating new column with the converted date to week name day - Mon, Tue, Wed, etc.")
        total_amount_df = create_column_with_names_of_the_days(df=filtered_df_sales_quntity_per_weight,
                                                               column=date_col)
        ...
        logger.info("Grouping by converted day_of_week and day_name columns "
                    "and creating new columns with max and min daily quantity per weight in their real values.")

        daily_distribution_df_quantity_per_weight = convert_to_pandas(do_transform_per_unit_of_measure(df=total_amount_df,
                                                                                                       unit_of_measure=unit_of_measure))
        logger.info("Successfully converted pyspark df to pandas dataframe")

        logger.info("Convert concrete columns from daily_distribution_df_quantity_per_weight df to pandas type")
        daily_distribution_df_quantity_per_weight = convert_to_pandas_type(df=daily_distribution_df_quantity_per_weight,
    ...
    def _visualize_prediction_plot(self, result_to_vis_dfs):
        ...
        daily_distribution_df_quantity_per_weight = result_to_vis_dfs

        draw_linear_regression_for_daily_purchased_quantity(df=daily_distribution_df_quantity_per_weight,
                                                            x_axis_column="day_of_week",
                                                            y_axis_column="min_daily_quantity_per_weight",
                                                            top="min",
                                                            unit_of_measure=unit_of_measure)

        draw_linear_regression_for_daily_purchased_quantity(df=daily_distribution_df_quantity_per_weight,
                                                            x_axis_column="day_of_week",
                                                            y_axis_column="max_daily_quantity_per_weight",
                                                            top="max",
                                                            unit_of_measure=unit_of_measure)